...
fn hande_cd(tree: &mut TreeIndex<Directory>, cur_node_id: Option<usize>, line: &str) -> Option<usize> {
	...
	if !dir_exists { //if it doesn't exist, create it
		let dir = Directory{
			name: dir_name.clone(),
			size: 0,
		};
		cur_node_id = tree.add_node(cur_node_id, dir);
	} else { //otherwise, get the node id for the existing directory
		//get the node id
		let node_id = tree.nodes.iter().position(|node| node.data.name == dir_name).unwrap();
...
fn handle_ls(tree: &mut TreeIndex<Directory>, cur_node_id: Option<usize>, line: &str) -> Option<usize> {
	//do nothing
	return cur_node_id;
}

fn handle_dir(tree: &mut TreeIndex<Directory>, cur_node_id: Option<usize>, line: &str) -> Option<usize> {
...
fn handle_file(tree: &mut TreeIndex<Directory>, cur_node_id: Option<usize>, line: &str) -> Option<usize> {
	...
	if !file_exists { //if it doesn't exist, create it
		let file = Directory{
			name: file_name.clone(),
			size: file_size,
		};
		tree.add_node(cur_node_id, file);
	}
...
fn apply_line(tree: &mut TreeIndex<Directory>, cur_node_id: Option<usize>, line: &str) -> Option<usize> {
	let reg_cd = Regex::new(r#"\$ cd ([/.[:alnum:]]+)"#).unwrap();
...
fn propagate_size(tree: &mut TreeIndex<Directory>, node_id: usize) {
...
fn main() {
	...
	// cur_node_id = apply_line(&mut tree, cur_node_id, lines[0].as_str());
	// cur_node_id = apply_line(&mut tree, cur_node_id, "$ cd home");
	// cur_node_id = apply_line(&mut tree, cur_node_id, "$ cd ..");
	// cur_node_id = apply_line(&mut tree, cur_node_id, "dir yolo");
	// cur_node_id = apply_line(&mut tree, cur_node_id, "$ cd home");
	// cur_node_id = apply_line(&mut tree, cur_node_id, "14848514 b.txt");
	...
	//set the size of the root node to 14
	//tree.get_node_mut(Some(0)).unwrap().data.size = 14;
